...
export default function RegisterPage() {
  ...
  const generateId = () => Date.now(); // simple unique id
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = { id: generateId(), username, email, password, role };
    ...
    try {
      ...
      if (res.ok && data.token) { // Check for 'token' in data for successful registration
  ...
  return (
    <main className="flex min-h-screen items-center backdrop-blur-sm justify-center bg-gray-950/0">
      {/* <GalaxyBackground/> */}
  
      <div
        className="w-96 space-y-4 border border-gray-800 bg-gray-900 p-6 rounded-xl shadow"
      >
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1 sr-only" htmlFor="email">Email</label>
            <input
              type="email"
              id="email"
              value={email}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-1 sr-only" htmlFor="role">Role</label>
            <select
              id="role"
              className="input w-full"
              value={role}
          ...
          <button
            type="submit"
            className="bg-blue-800 border border-blue-600 hover:bg-blue-900 p-2 cursor-pointer rounded-md w-full disabled:opacity-50"
            disabled={loading}
          >
        ...
        <div className="space-y-3">
          <button
            ...
            className="w-full cursor-pointer flex items-center justify-center py-2 px-4 rounded-lg bg-blue-950 hover:bg-blue-950/80 border border-blue-900 text-white font-semibold shadow-sm transition-all duration-200"
          >
          ...
          onMouseLeave={()=>setNoticeVisible(false)}>

          <button
            disabled={true}
            onClick={handleFacebookLogin}
            className="w-full relative flex cursor-pointer disabled:border disabled:border-gray-500 disabled:bg-gray-600 disabled:cursor-not-allowed items-center justify-center py-2 px-4 rounded-lg bg-blue-700 hover:bg-blue-800 text-white font-semibold shadow-sm transition-all duration-200"
          >
            <svg className="w-5 h-5 mr-2" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            ...
            <div
              onMouseEnter={()=>setNoticeVisible(true)}
              onMouseLeave={()=>setNoticeVisible(false)}
              className={`${!noticeVisible && "hidden"} absolute top-10 z-10 bg-amber-500 rotate-45 text-black border-l-2 border-t-2 border-black p-2`}></div>
            <div 
              onMouseEnter={()=>setNoticeVisible(true)}
              onMouseLeave={()=>setNoticeVisible(false)}
              className={`${!noticeVisible && "hidden"} absolute top-12 bg-amber-500 text-black border-2 border-black p-2 rounded-md`}>Not available for the moment</div>
          </button>
          </div>
        </div>
